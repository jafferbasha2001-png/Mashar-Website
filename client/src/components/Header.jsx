...
export default function Header({ language, onLanguageChange }) {
  ...
  return <header className="site-header" id="siteHeader"><div className="container nav-wrap">
    ...
    <nav className={`main-nav ${open ? 'open' : ''}`}>{links.map(([id, label]) => <a key={id} href={`#${id}`} onClick={() => setOpen(false)}>{language === 'ar' ? id === 'home' ? 'الرئيسية' : id === 'customers' ? 'العملاء' : label : label}</a>)}<button className="language-btn" type="button" onClick={onLanguageChange}>{language === 'en' ? 'العربية' : 'English'}</button></nav>
  </div></header>;
}